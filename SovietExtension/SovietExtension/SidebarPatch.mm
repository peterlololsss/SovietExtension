...
public:
    ...
    void InvalidateOwner(const OwnerToken &token) override {
        ...
        if (tokenObserver) {
            tokenObserver({OwnerTokenEventKind::invalidated, token});
        }
    }
    ...
    void PublishOwner(const OwnerToken &token) override {
        ...
        if (tokenObserver) {
            tokenObserver({OwnerTokenEventKind::published, token});
        }
    }
    ...
    OwnerTokenObserver tokenObserver;
...
void OwnerBridge::setTokenObserver(OwnerTokenObserver observer) {
    std::lock_guard<std::recursive_mutex> lock(implementation_->mutex);
    implementation_->tokenObserver = std::move(observer);
}
