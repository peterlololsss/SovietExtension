...
inline constexpr std::size_t kNormalPatchStageCount = 5;
...
constexpr bool ShouldBypassInventoryApply(
    const InstallDecision &decision) noexcept {
    return decision.bypassInventoryApply;
}
...
// ---- SidebarPatchOwnerBridgeProtocol ---------------------------

namespace ym::sidebar::patch_guard {

enum class OwnerTokenEventKind : unsigned char {
    published,
    invalidated,
};

struct OwnerTokenEvent final {
    OwnerTokenEventKind kind{OwnerTokenEventKind::invalidated};
    OwnerToken token{};
};

using OwnerTokenObserver = std::function<void(const OwnerTokenEvent &)>;

class OwnerBridgeProtocol {
public:
    virtual ~OwnerBridgeProtocol() = default;

    [[nodiscard]] virtual OwnerToken currentToken() const noexcept = 0;
    virtual void setTokenObserver(OwnerTokenObserver observer) = 0;
};
...
public:
    ...
    [[nodiscard]] OwnerToken currentToken() const noexcept override;
    void setTokenObserver(OwnerTokenObserver observer) override;
