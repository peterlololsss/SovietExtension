...
struct Configuration {
    ...
    // 关闭时不改写微信的布局模式字节，也不安装展开相关的补丁点，
    // 侧边栏保持微信原生的折叠形态。默认关闭。
    bool exposeSecondaryEntries{false};
...
inline PropertyList ToPropertyList(const Configuration &configuration) {
    ...
    propertyList.exposeSecondaryEntries = normalized.exposeSecondaryEntries;
    return propertyList;
}
...
inline ValidationResult NormalizeStoredConfiguration(
    ...
    configuration.exposeSecondaryEntries = stored.exposeSecondaryEntries;
...
inline ValidationResult ValidateSaveInput(const PropertyList &input) {
    ...
    configuration.exposeSecondaryEntries = input.exposeSecondaryEntries;
    if (const std::optional<ValidationError> error =
            detail::ValidatePrimaryVisibility(configuration)) {
        return ValidationResult::Failure(*error);
    }
...
public:
    ...
    [[nodiscard]] bool exposeSecondaryEntries() const noexcept {
        return draft_.exposeSecondaryEntries;
    }

    void setExposeSecondaryEntries(bool expose) noexcept {
        draft_.exposeSecondaryEntries = expose;
    }

    [[nodiscard]] bool exposeDirty() const noexcept {
        return draft_.exposeSecondaryEntries != baseline_.exposeSecondaryEntries;
    }

    [[nodiscard]] bool dirty() const noexcept {
        return visibilityDirty() || orderDirty() || exposeDirty();
    }

    // 布局模式只在微信启动时决定一次，所以改这个开关和改顺序一样要重启。
    [[nodiscard]] bool requiresRestart() const noexcept {
        return orderDirty() || exposeDirty();
    }
