...
@property(nonatomic, getter=isExposingSecondaryEntries)
    BOOL exposeSecondaryEntries;
...
inline bool ParseConfiguration(NSDictionary<NSString *, id> *dictionary,
    ...
    if (![dictionary isKindOfClass:NSDictionary.class] ||
        (dictionary.count != 4 && dictionary.count != 5)) {
        return false;
    }
    ...
    id exposeValue = dictionary[@"exposeSecondaryEntries"];
    if (exposeValue != nil &&
        (![exposeValue isKindOfClass:NSNumber.class] ||
         !NSNumberIsBoolean(exposeValue))) {
        return false;
    }
    ...
    parsed.exposeSecondaryEntries =
        exposeValue != nil && [static_cast<NSNumber *>(exposeValue) boolValue];
...
inline NSDictionary<NSString *, id> *FoundationConfiguration(
    ...
    return @{
        ...
        @"exposeSecondaryEntries" : @(configuration.exposeSecondaryEntries),
    };
}
...
- (BOOL)requiresRestart {
    return ym::sidebar::objc_bridge::Model(_storage)->requiresRestart();
}

- (BOOL)isExposingSecondaryEntries {
    return ym::sidebar::objc_bridge::Model(_storage)->exposeSecondaryEntries();
}

- (void)setExposeSecondaryEntries:(BOOL)expose {
    ym::sidebar::objc_bridge::Model(_storage)
        ->setExposeSecondaryEntries(expose == YES);
}
