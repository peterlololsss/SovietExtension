...
// In WeChat's native expanded layout the primary order getter omits 发现
// entirely, because the five 发现与服务 entries are themselves sidebar items.
// Our mode flip necessarily lands *after* the primary list has been built, so
// the collapsed-mode 发现 entry survives and visually duplicates the strip
// below it. Hide it explicitly to match the native expanded layout.
//
// Only ever runs when we actually forced the flip; if WeChat was already
// expanded it built the correct list itself and there is nothing to hide.
// Re-applied on every layout pass because WeChat re-shows primary items during
// its own responsive pass.
//
// Apply the user's 发现 checkbox to the native primary type-3 entry.
//
// 发现 is managed for VISIBILITY only. WeChat's own primary order getter always
// appends type 3 last (0xE55970), so it is deliberately not part of the
// reorderable primary order, and the manager shows it as a pinned tail row.
//
// Only runs under our forced expanded layout; if WeChat is natively expanded it
// built the correct primary list itself and 发现 was never in it.
static void YMNavigationSidebarApplyDiscoverEntryVisibility(
    ...
    // 发现 is now a managed entry: the user's checkbox is authoritative.
...
static bool YMNavigationSidebarRunResponsiveLayout(void *owner) {
    ...
    // After the managed primary items settle, drop the now-redundant 发现 entry
    // so the forced expanded layout matches WeChat's native one.
    YMNavigationSidebarApplyDiscoverEntryVisibility(
        owner, configuration, scope.saved);
...
struct YMNavigationSidebarPatchContext {
    std::array<YMNavigationSidebarPatchStage,
        ...
        stages{};
    std::array<YMNavigationSidebarCallGuard, 15> callGuards{};
