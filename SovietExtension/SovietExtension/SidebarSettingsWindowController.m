...
typedef struct {
    ...
    CGFloat exposeSwitchHeight;
    CGFloat exposeSwitchGap;
...
static const YMSidebarSettingsDesignMetrics kYMSidebarDesignMetrics = {
    ...
    .subtitleHeight = 16.0,
    .subtitleCardGap = 8.0,
    .exposeSwitchHeight = 20.0,
    .exposeSwitchGap = 12.0,
...
@property(nonatomic, strong) NSButton *exposeSwitch;
...
- (void)ym_buildInterfaceInView:(NSView *)contentView
{
    ...
    self.exposeSwitch = [[NSButton alloc] initWithFrame:NSZeroRect];
    [self.exposeSwitch setButtonType:NSButtonTypeSwitch];
    self.exposeSwitch.title = @"把「发现与服务」展开到侧边栏";
    self.exposeSwitch.font =
        YMFontForTypographyToken(YMSidebarTypographyRowTitle);
    self.exposeSwitch.target = self;
    self.exposeSwitch.action = @selector(ym_toggleExposeSecondaryEntries:);
    self.exposeSwitch.toolTip =
        @"关闭时微信侧边栏保持原生形态，插件不改写布局；开启后朋友圈、视频号等"
        @"成为独立入口，可单独开关与排序。切换后需重启微信生效。";
    [self.scrollDocumentView addSubview:self.exposeSwitch];
...
- (void)ym_layoutInterface
{
    ...
    CGFloat requiredHeight = kYMSidebarDesignMetrics.bodyTopPadding +
        ...
        kYMSidebarDesignMetrics.exposeSwitchHeight +
        kYMSidebarDesignMetrics.exposeSwitchGap +
    ...
    top -= kYMSidebarDesignMetrics.subtitleHeight +
        kYMSidebarDesignMetrics.subtitleCardGap;
    self.exposeSwitch.frame = NSMakeRect(
        headingInset, top - kYMSidebarDesignMetrics.exposeSwitchHeight,
        headingWidth, kYMSidebarDesignMetrics.exposeSwitchHeight);
    top -= kYMSidebarDesignMetrics.exposeSwitchHeight +
        kYMSidebarDesignMetrics.exposeSwitchGap;
                   ...
                   row:(NSInteger)rowIndex
{
    ...
    // 微信把「发现」钉在主组末位（顺序表只在折叠模式下才含它），它两个方向都
    // 不能移动。既然移不了，就别画拖拽把手——有把手却拖不动比没有更难理解。
    const BOOL rowCanMove = [row[@"canMoveUp"] boolValue] ||
                            [row[@"canMoveDown"] boolValue];
    dragHandle.hidden = !rowCanMove;
...
- (void)ym_toggleExposeSecondaryEntries:(NSButton *)sender
{
    if (self.draft == nil) {
        sender.state = NSControlStateValueOff;
        return;
    }
    self.draft.exposeSecondaryEntries =
        sender.state == NSControlStateValueOn;
    [self ym_updateSecondaryGroupAvailability];
    [self ym_updateStatus];
}

// 关闭展开时插件不改写微信布局：「发现与服务」在折叠布局下不是侧边栏条目，
// 顺序也由微信自己决定。因此该组整体不可操作，两组的排序按钮一并停用，只留
// 主导航的显示开关可用。
- (void)ym_updateSecondaryGroupAvailability
{
    BOOL expose = self.draft.isExposingSecondaryEntries;
    self.secondaryTable.enabled = expose;
    self.secondaryCard.alphaValue = expose ? 1.0 : 0.45;
    if (!expose) {
        self.primaryMoveUpButton.enabled = NO;
        self.primaryMoveDownButton.enabled = NO;
        self.secondaryMoveUpButton.enabled = NO;
        self.secondaryMoveDownButton.enabled = NO;
    }
}
            ...
            pasteboardWriterForRow:(NSInteger)rowIndex
{
    ...
    NSDictionary<NSString *, id> *row = rows[(NSUInteger)rowIndex];
    // 固定行（发现被微信钉在末位）两个方向都不能移动，那就不该能拖起来——
    // 否则拖得动、放下去却没反应，看着像坏了。
    if (![row[@"canMoveUp"] boolValue] && ![row[@"canMoveDown"] boolValue]) {
        return nil;
    }
...
- (void)ym_reloadDraftFromManager
{
    ...
    self.inlineMessage = self.draft == nil
        ...
        : nil;
    self.exposeSwitch.state = self.draft.isExposingSecondaryEntries
        ? NSControlStateValueOn
        : NSControlStateValueOff;
    [self ym_updateSecondaryGroupAvailability];
                           ...
                           groupTitle:(NSString *)groupTitle
{
    ...
    // 选中的是固定行（发现）时，这两个按钮对它永远无效，直接收起来而不是留个
    // 灰着的按钮——和该行不画拖拽把手保持一致。
    const BOOL pinnedSelection = row != nil && !up.enabled && !down.enabled;
    up.hidden = pinnedSelection;
    down.hidden = pinnedSelection;
    NSString *title = row[@"title"];
...
- (void)ym_updateStatus
{
    ...
    } else if (self.draft.requiresRestart) {
        self.statusLabel.stringValue = @"有未保存的修改；保存后重启微信生效";
        self.statusLabel.textColor = YMColorForDesignToken(
...
- (void)ym_save:(NSButton *)sender
{
    ...
    // 保存后 draft 会重新基线化，dirty 标记随之清空，所以先记下来。
    const BOOL needsRestart = self.draft.requiresRestart;
    NSError *error = nil;
    ...
    [self ym_promptRestartWeChat];
}

// 顺序和展开开关都只在微信启动时读取一次，保存完直接问一句要不要重启。
// 用 sheet 而不是 runModal：runModal 会阻塞当前 runloop，自动化场景下永远等不到
// 回应，保存路径就挂死了。
- (void)ym_promptRestartWeChat
{
    ...
    alert.informativeText =
        @"顺序和展开设置需要重启微信后生效。现在重启吗？";
    [alert addButtonWithTitle:@"重启微信"];
    ...
    [alert beginSheetModalForWindow:window
        ...
        [self ym_restartWeChat];
    }];
}

- (void)ym_restartWeChat
{
    dispatch_async(
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            task.arguments = @[
                @"-c",
                @"killall WeChat; sleep 0.5; open /Applications/WeChat.app",
            ];
            @try {
                [task launch];
